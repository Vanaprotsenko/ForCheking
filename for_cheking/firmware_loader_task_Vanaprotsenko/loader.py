...
class Loader:
    ...
    def get_device(self) -> Device:
        ...
        category_choice = input("Enter the number of desired category: ")
        ...
        if category_choice in category_mapping:
            ...
            divece_choice = input("Enter the number of desired device: ")
            try:
    ...
    def get_firmware(self):
        ...
        version = input("Enter firmware version: ")
        region = input("Enter region: ")
        subtype = self.choose_subtype()
        id_type = self.input_device_id()
        
        flash_in_color = self.color_to_flash()

        firmware_name = f"[{device_name}][{version}][{region}][{subtype}][{id_type}].hex"
        if flash_in_color:
    ...
    def color_to_flash(self) ->bool:
        while True:
            flash_color = input("Do you want to flash in color? (y/n): ").lower()

            if flash_color in ['y', 'n']:
                return flash_color == 'y'
            else:
        ...
        while True:
            ...
            try:
                ...
                if 0 <= subtype_int <= 255:
                    ...
                    print("Invalid range. Subtype should be a number between 0 and 255. Please try again.")
            except ValueError:
                print("Invalid input. Subtype should be a number. Please try again.")
